...
export default function Outcome() {
  ...
  const onSubmit = ({ actualDate, note, amount, classificationId }) => {
    submitLoading();
    service({
      url: edit ? `/transaction/outcome/${edit}` : "/transaction/outcome",
      data: {
    ...
    service
      .get("/transaction/outcome", {
        params: {
          ...
          year,
          month,
          classificationId: classification ? classification : null,
        },
      })
      .then((response) => {
        if (mounted.current) {
  ...
  useEffect(() => {
    ...
    webContext.dispatch({ type: "titlePage", value: "Uang Keluar (Kredit)" });
    getOptions();
    return () => {
  ...
  useEffect(() => {
    getData();
  }, [activePage, month, year, classification]);
        ...
        <Fragment>
              ...
              value={[
                {
                  ...
                  value:
                    baseURL +
                    `/export/outcome/${month}/${year}/${classification || 0}`,
                },
              ]}
          ...
          <div className="flex mb-3 flex-wrap">
            <Dropdown
              selected={[
                ...
                ...options,
              ].findIndex((predicate) => predicate.value === classification)}
              placeholder="Semua"
              className="bg-blue-600 w-24 flex justify-between mr-2 mb-1"
              onClick={(item) => setClassification(item.value)}
              value={[{ value: undefined, label: "Semua" }, ...options]}
            />
            <Dropdown
          ...
          <Table>
            <thead className="bg-primary-100">
              <tr>
                <Th className="w-2/12">TGL</Th>
                <Th className="w-1/12">KODE</Th>
                <Th className="w-5/12">KETERANGAN</Th>
                <Th className="w-2/12">NILAI</Th>
            ...
            <tbody>
              {data.map((item, index) => (
                <tr key={`${index}`}>
                  ...
                  <Td className="text-center">{item.classification.code}</Td>
                  <Td>{item.note}</Td>
                  ...
                  <Td className="text-center">
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setValue("classificationId", item.classificationId);
                        setEdit(item.id);
                        modalRef.current.show();
                      }}
                    >
